...
void *Thread(void *arg)
{
    ...
    for (x = start; x < end; x++)
    {
        ...
        for (y = 0; y < second_matrix_dimensions[1]; y += 8)
        {
            __m256i sum = _mm256_setzero_si256();
            for (z = 0; z < second_matrix_dimensions[0]; z++)
            {
                __m256i vec_a = _mm256_set1_epi32(first_matrix[x][z]);
                __m256i vec_b = _mm256_loadu_si256((__m256i *) &second_matrix[z][y]);
                __m256i vec_res = _mm256_mullo_epi32(vec_a, vec_b);
                sum = _mm256_add_epi32(sum, vec_res);
            }

            _mm256_storeu_si256((__m256i*)&result_matrix[x][y], sum);
        }
    }
